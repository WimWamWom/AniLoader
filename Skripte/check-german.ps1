...
Write-Host 'AniLoader - Check German Episodes' -ForegroundColor Cyan
...
try {
    ...
    while ($true) {
        ...
        if ([string]$statusResponse.status -ne 'running') {
            Write-Host '[OK] System ist frei, starte German-Check...'
            break
        }
    ...
    Write-Host "Starte German-Check via $API_ENDPOINT..."
    [void](Invoke-AniLoaderApi -Method POST -Uri "$API_ENDPOINT/start_download" -Body @{ mode = 'german' })
    ...
    $parsedEpisodes = @(Parse-GermanEpisodes -LogContent $logContent)
    $errorCount = Get-LogMatchCount -LogContent $logContent -Pattern '\[ERROR\]'
    $germanEpisodesCount = $parsedEpisodes.Count

    Write-Host ''
    Write-Host '=== ZUSAMMENFASSUNG ==='
    Write-Host "Neue deutsche Episoden gefunden: $germanEpisodesCount"
    Write-Host "Fehler: $errorCount"

    if ($germanEpisodesCount -gt 0) {
        $summary = "✅ **$germanEpisodesCount neue deutsche Episode(n) heruntergeladen!**"
        $errorInfo = ''
        if ($errorCount -gt 0) {
            $errorInfo = "⚠️ $errorCount Fehler aufgetreten"
        }

        if (Send-DiscordMultiEmbed -Title '🇩🇪 AniLoader - Deutsche Episoden Check' -Summary $summary -Color 3066993 -ErrorInfo $errorInfo -Episodes $parsedEpisodes -WebhookUrls $DISCORD_WEBHOOK_URLS -FooterText 'Unraid AniLoader' -ContinuationPrefix '🇩🇪') {
