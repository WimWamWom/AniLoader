...
def insert_anime(url, title=None):
    ...
    try:
        ...
        c.execute("INSERT OR IGNORE INTO anime (url, title) VALUES (?, ?)", (url, title))
        conn.commit()
        log(f"[DB] Inserted (or existed): {title} / {url}")
        return True
    except Exception as e:
        log(f"[DB-ERROR] {e}")
        return False
...
def load_anime():
    ...
    for row in rows:
        entry = {
            ...
            "fehlende_deutsch_folgen": json.loads(row[5] or "[]"),
            "last_film": row[6],
            "last_episode": row[7],
            "last_season": row[8]
        }
...
def api_database():
    ...
    sql = "SELECT id, title, url, complete, deutsch_komplett, fehlende_deutsch_folgen, last_film, last_episode, last_season FROM anime"
    params = []
    where_clauses = []
    if q:
