...
NEW_GERMAN_COUNT=$(echo "$LOG_CONTENT" | grep -c "\[GERMAN\].*erfolgreich auf deutsch")
ERROR_COUNT=$(echo "$LOG_CONTENT" | grep -c "\[ERROR\]")

# Erstelle Liste ALLER neuen deutschen Episoden (kein Limit)
NEW_EPISODES=$(echo "$LOG_CONTENT" | grep "\[GERMAN\].*erfolgreich auf deutsch" | sed 's/.*\[GERMAN\] //')
