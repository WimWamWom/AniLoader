# check-german.ps1 - Prüft auf neue deutsche Episoden (PowerShell Version)
...
# Write-Host "`nStarte German-Check via $API_ENDPOINT..." -ForegroundColor Yellow
# $body = '{"mode":"german"}'
...
# Parsing für GERMAN Modus - nur German Dub Downloads zählen
$parsedEpisodes = @()
$currentUrl = ""
...
foreach ($line in $lines) {
    ...
    # [VERIFY] ... wurde erfolgreich erstellt (nur wenn German Dub)
    elseif ($line -match '\[VERIFY\].*wurde.*erfolgreich.*erstellt' -and $currentUrl -and $currentIsGerman) {
        ...
        if ($url -match '/(?:anime|serie)/stream/([^/]+)/staffel-(\d+)/episode-(\d+)') {
            ...
            Write-Host "  [GEFUNDEN - German Dub] $formatted" -ForegroundColor Green
...
if ($germanEpisodesCount -gt 0) {
    $summary = "[OK] **$germanEpisodesCount neue deutsche Episode(n) gefunden!**"
    ...
    Send-DiscordGroupedEmbed -Title "[DE] AniLoader - Deutsche Episoden Check" -Summary $summary -Color 3066993 -ErrorInfo $errorInfo -Episodes $parsedEpisodes
} else {
