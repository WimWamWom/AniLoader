...
ANIME_TXT = os.path.join(BASE_DIR, "Download.txt")
...
current_download = {"status": "idle", "title": None}
...
def download_episode(series_title, episode_url, season, episode, anime_id, german_only=False):
    ...
    if not german_only and episode_already_downloaded(series_folder, season, episode):
...
def start_download_mode(mode):
    ...
    for anime in anime_list:
        ...
        current_download["status"] = "running"
        current_download["title"] = series_title
        ...
        if mode == "german":
            ...
            for url in fehlende:
                ...
                download_episode(series_title, url, season, episode, anime_id, german_only=True)
        elif mode == "new":
            ...
            update_anime(anime_id, complete=True)
    current_download["status"] = "idle"
    current_download["title"] = None
    log("[FERTIG] Download abgeschlossen.")
...
def start_download():
    mode = request.json.get("mode", "default").lower()
    threading.Thread(target=start_download_mode, args=(mode,), daemon=True).start()
...
def get_database():
    ...
    filter_complete = request.args.get("complete", "all")

    anime_list = load_anime()
    filtered = []
    for anime in anime_list:
        if search and search not in anime["title"].lower():
            continue
        if filter_complete == "yes" and not anime["complete"]:
            continue
        if filter_complete == "no" and anime["complete"]:
            continue
